...
void motoresPWM()
{
  ...
  // ^^ NO SE SI USAR LAS DOS LINEAS DE ARRIBA O LAS DOS DE ABAJO
  // x_pwm = map( (int)  abs(ang_x), 0, 90, 0, 1023);
  // y_pwm = map( (int)  abs(ang_y), 0, 90, 0, 1023);
  // analogWrite(PIN_QUE_CORRESPONDA, x_pwm);// ^^ no se cual va en PIN_QUE_CORRESPONDA
  /* ^^ A PARTE NO SE SI DEBE ESTAR SIEMPRE SETEADO POR LAS DUDAS BAJO BAJO PARA LOS PINES DEL OTRO,
  PARA QUE AL HACER EL analogWrite NO IMPULSAR DE MANERA INCORRECTA EL MOTOR RESTANTE */
  analogWrite(5, y_pwm);// ^^ no se por que 5
}
